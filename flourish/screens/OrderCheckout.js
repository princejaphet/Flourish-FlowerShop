...
const OrderCheckout = ({ route, navigation }) => {
  ...
  // MODIFIED: Destructure 'variation' instead of 'size' to match data from ProductDetail screen
  const { product, quantity, variation, note, isNewUser } = orderDetails;
  // MODIFIED: Use variation.name for the display size
        ...
        if (user) {
          ...
          try {
            ...
            if (userDoc.exists()) {
              ...
              setAddress(userData.address || 'No address provided');
  ...
  const handleSelectProofImage = async () => {
    ...
    if (!pickerResult.canceled) {
      setProofOfPaymentImage(pickerResult.assets[0].uri);
    }
  };
  ...
  const handlePlaceOrder = async () => {
    ...
    if (!name || !address || !contactNumber || address.startsWith('No address')) {
        Alert.alert("Missing Information", "Please provide a complete shipping address.");
        return;
    }
    
    // NEW: Add validation for product size selection
    if (product.variations && product.variations.length > 0) {
      if (!variation || !variation.name || variation.name === 'Default') {
        Alert.alert("Selection Required", "Please select a size before placing your order.");
        return;
      }
    }
    ...
    try {
      ...
      const orderData = {
        ...
        deliveryAddress: { name, phone: contactNumber, address },
        product: {
          name: mainProduct.name,
          price: mainProduct.price,
          originalPrice: mainProduct.originalPrice,
          quantity: mainProduct.quantity,
          addons: mainProduct.addons || [],
          // 'size' field will be added conditionally below
      ...
      // NEW: Conditionally add the size field to prevent 'undefined' error
      if (variation && variation.name && variation.name !== 'Default') {
        orderData.product.size = variation.name;
      }
    ...
    <Modal animationType="fade" transparent={true} visible={isSuccessModalVisible} onRequestClose={() => setSuccessModalVisible(false)}>
      <View style={styles.modalOverlay}>
        <View style={styles.successModalContent}>
          ...
          <TouchableOpacity style={styles.trackOrderButton} onPress={() => { setSuccessModalVisible(false); navigation.replace('OrderTracking', { orderId: newOrderId }); }}>
            <Text style={styles.trackOrderButtonText}>Track Order</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.backToHomeButton} onPress={() => { setSuccessModalVisible(false); navigation.navigate('Homepage'); }}>
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView contentContainerStyle={styles.container}>
            ...
            {orderItems.map((item, index) => (
              <View key={index.toString()} style={styles.productRow}>
                ...
                <View style={styles.productInfo}>
                  ...
                  {/* MODIFIED: Display size only if it exists */}
        ...
        <View style={styles.sectionContainer}>
          ...
          <View>
            <View style={styles.deliveryRow}>
              ...
              <TouchableOpacity onPress={() => setShowDeliveryOptions(!showDeliveryOptions)} style={styles.datePickerButton}>
                <Text style={styles.datePickerText}>{deliveryOption}</Text>
                <Feather name={showDeliveryOptions ? "chevron-up" : "chevron-down"} size={20} color={colors.subText} />
              </TouchableOpacity>
            </View>
            {showDeliveryOptions && (
        ...
        <View style={styles.sectionContainer}>
          ...
          <TextInput style={styles.notesInput} placeholder="Add special instructions for your order..." placeholderTextColor={colors.subText} value={orderNotes} onChangeText={setOrderNotes} multiline />
        </View>

        <View style={styles.sectionContainer}>
          <Text style={styles.sectionTitle}>Payment Methods</Text>
          <TouchableOpacity onPress={() => setPaymentMethod('Cash on Delivery')} style={styles.paymentOption}>
            <Icon name={paymentMethod === 'Cash on Delivery' ? "radiobox-marked" : "radiobox-blank"} size={24} color={colors.primary} />
            <Text style={styles.paymentOptionText}>Cash On Delivery </Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => setPaymentMethod('GCash')} style={styles.paymentOption}>
            <Icon name={paymentMethod === 'GCash' ? "radiobox-marked" : "radiobox-blank"} size={24} color={colors.primary} />
          ...
          {paymentMethod === 'GCash' && (
            <View style={styles.paymentInstructions}>
              ...
              <View style={styles.instructionRow}>
                <Icon name="file-document-outline" size={20} color={colors.subText} />
                <View style={styles.instructionTextContainer}>
                  <Text style={styles.instructionText}>Send payment to:</Text>
                  <Text style={styles.instructionDetail}>Name: Flourish Flower</Text>
                  <Text style={styles.instructionDetail}>Mobile No: 09219348606</Text>
                </View>
              </View>
              <Image source={qrCodeImage} style={styles.qrCode} />
              <Text style={styles.instructionFooter}>
                After sending, upload a screenshot and/or enter your GCASH reference number below.
              </Text>

              {proofOfPaymentImage && (
                <View style={styles.imagePreviewContainer}>
                  <Image source={{ uri: proofOfPaymentImage }} style={styles.imagePreview} />
                  <TouchableOpacity style={styles.removeImageButton} onPress={() => setProofOfPaymentImage(null)}>
              ...
              <TouchableOpacity style={styles.uploadButton} onPress={handleSelectProofImage}>
                <Text style={styles.uploadButtonText}>
                  {proofOfPaymentImage ? 'Change Proof of Payment' : 'Upload Proof of Payment'}
                </Text>
              </TouchableOpacity>
        ...
        <View style={styles.summaryContainer}>
          ...
          {isNewUser && (
            <View style={styles.summaryRow}>
                <Text style={styles.summaryText}>New User Discount (5%):</Text>
                <Text style={[styles.summaryValue, {color: colors.discount}]}>- ₱{newUserDiscount.toFixed(2)}</Text>
            </View>
          )}
          ...
          <TouchableOpacity style={styles.summaryRow} onPress={() => setVoucherModalVisible(true)}>
            <Text style={styles.summaryText}>Voucher:</Text>
            <Text style={[styles.summaryValue, {color: colors.primary}]}>{appliedVoucher ? `- ₱${voucherDiscount.toFixed(2)}` : 'Apply Voucher >'}</Text>
          </TouchableOpacity>
      ...
      <View style={styles.footer}>
        ...
        <TouchableOpacity
          style={[styles.placeOrderButton, (isLoading || isUploading) && styles.disabledButton]}
          onPress={handlePlaceOrder}
          disabled={isLoading || isUploading}
        >
          {isUploading ? (
            <Text style={styles.placeOrderButtonText}>Uploading Proof...</Text>
          ) : isLoading ? (
      ...
      <Modal animationType="slide" transparent={true} visible={isVoucherModalVisible} onRequestClose={() => setVoucherModalVisible(false)}>
        <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPressOut={() => setVoucherModalVisible(false)}>
...
const getStyles = (colors) => StyleSheet.create({
  ...
  progressContainer: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', padding: 20, backgroundColor: colors.cardBackground },
  stepText: { fontSize: 14, color: colors.subText },
  activeStepText: { color: colors.primary, fontWeight: 'bold' },
  sectionContainer: { marginTop: 12, backgroundColor: colors.cardBackground, padding: 15 },
  sectionTitle: { fontSize: 16, fontWeight: 'bold', color: colors.text, marginBottom: 15 },
  addressBox: { flexDirection: 'row', alignItems: 'center', padding: 10, borderRadius: 8, borderWidth: 1, borderColor: colors.border },
  addressIcon: { marginRight: 15 },
  addressDetails: { flex: 1 },
  addressName: { fontSize: 15, fontWeight: 'bold', color: colors.text, marginBottom: 4 },
  addressText: { fontSize: 14, color: colors.subText },
  ...
  productPrice: { fontSize: 15, fontWeight: 'bold', color: colors.text },
  ...
  summaryContainer: { backgroundColor: colors.cardBackground, padding: 15, marginTop: 12 },
  summaryRow: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 8 },
  summaryText: { fontSize: 15, color: colors.subText },
  summaryValue: { fontSize: 15, color: colors.text, fontWeight: '500' },
  divider: { height: 1, backgroundColor: colors.border, marginVertical: 10 },
  totalText: { fontSize: 16, color: colors.text, fontWeight: 'bold' },
  totalValue: { fontSize: 18, color: colors.primary, fontWeight: 'bold' },
  footer: { position: 'absolute', bottom: 0, left: 0, right: 0, flexDirection: 'row', alignItems: 'center', backgroundColor: colors.cardBackground, borderTopWidth: 1, borderTopColor: colors.border, paddingHorizontal: 20, paddingVertical: 10, paddingBottom: 20 },
  footerPrice: { flex: 1 },
  footerTotalLabel: { fontSize: 14, color: colors.subText },
  ...
  voucherModalContent: { backgroundColor: colors.cardBackground, borderRadius: 20, padding: 20, width: '90%' },
  modalTitle: { fontSize: 20, fontWeight: 'bold', color: colors.text, marginBottom: 20, textAlign: 'center' },
  voucherList: { maxHeight: 250 },
  voucherItem: { flexDirection: 'row', alignItems: 'center', paddingVertical: 15, borderBottomWidth: 1, borderBottomColor: colors.border },
  voucherIcon: { marginRight: 15 },
  voucherInfo: { flex: 1 },
  voucherCode: { fontSize: 16, fontWeight: 'bold', color: colors.text },
  voucherDesc: { fontSize: 14, color: colors.subText, marginTop: 4 },
  successModalContent: { width: '90%', backgroundColor: colors.cardBackground, borderRadius: 20, padding: 30, alignItems: 'center', shadowColor: colors.shadowColor, shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.25, shadowRadius: 4, elevation: 5 },
  ...
  orderPlacedTitle: { fontSize: 24, fontWeight: 'bold', color: colors.text },
  orderPlacedSubtitle: { fontSize: 15, color: colors.subText, textAlign: 'center', marginVertical: 15, lineHeight: 22 },
  ...
  trackOrderButtonText: { color: '#fff', fontSize: 16, fontWeight: 'bold' },
  backToHomeButton: { backgroundColor: 'transparent', paddingVertical: 14, paddingHorizontal: 40, borderRadius: 30, marginTop: 10, width: '100%', alignItems: 'center' },
  backToHomeButtonText: { color: colors.primary, fontSize: 16, fontWeight: 'bold' },
  deliveryRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: colors.border },
  deliveryLabel: { fontSize: 15, color: colors.text, fontWeight: '500' },
  datePickerButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderWidth: 1, borderColor: colors.border, borderRadius: 8, paddingVertical: 12, paddingHorizontal: 15, width: '60%', backgroundColor: colors.inputBackground },
  datePickerText: { fontSize: 15, color: colors.text },
  deliveryDropdown: { alignSelf: 'flex-end', width: '60%', marginTop: 2, backgroundColor: colors.cardBackground, borderRadius: 8, borderWidth: 1, borderColor: colors.border, elevation: 3, shadowColor: colors.shadowColor, shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4 },
  deliveryOption: { paddingVertical: 12, paddingHorizontal: 15, borderBottomWidth: 1, borderBottomColor: colors.border },
  deliveryOptionText: { fontSize: 15, color: colors.text },
  paymentOption: { flexDirection: 'row', alignItems: 'center', paddingVertical: 15 },
  paymentOptionText: { fontSize: 16, color: colors.text, marginLeft: 15 },
  gcashLogo: { width: 25, height: 25, resizeMode: 'contain', marginLeft: 15 },
  paymentInstructions: { marginTop: 15, padding: 15, backgroundColor: colors.inputBackground, borderRadius: 8 },
  instructionsTitle: { fontSize: 16, fontWeight: 'bold', color: colors.text, marginBottom: 15 },
  instructionRow: { flexDirection: 'row', alignItems: 'flex-start', marginBottom: 15 },
  instructionTextContainer: { marginLeft: 10 },
  instructionText: { fontSize: 15, color: colors.text },
  instructionDetail: { fontSize: 14, color: colors.subText, marginTop: 4 },
  qrCode: { width: 150, height: 150, alignSelf: 'center', marginVertical: 15 },
  instructionFooter: { fontSize: 14, color: colors.subText, textAlign: 'center', marginBottom: 15 },
  ...
  uploadButtonText: { color: colors.primary, fontSize: 15, fontWeight: 'bold' },
  referenceInput: { backgroundColor: colors.cardBackground, borderWidth: 1, borderColor: colors.border, borderRadius: 8, padding: 12, color: colors.text, fontSize: 15 },
  imagePreviewContainer: { alignItems: 'center', marginBottom: 15, },
  imagePreview: { width: 150, height: 150, borderRadius: 8, borderWidth: 1, borderColor: colors.border, },
